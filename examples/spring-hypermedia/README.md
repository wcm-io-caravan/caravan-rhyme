...
Until this branch is merged to develop, you need to build this example from source to run it.
...
The **docs** icons in the HAL browser will link to a context-sensitive documentation for each resource and relation.
...
`@HalApiInterface` definition, and 'References' directs you to the server-side implementation.

It's one of the key advantages of **Rhyme** that the structure of the API's resources is very clearly defined in these `@HalApiInterface`,
...
also also the single source for the generated documentation that is automatically linked from your resources with the `curies` relation.
...
to access the resources under test. This allows the same set of tests to be run twice: directly testing the server-side implementations of those interfaces, or by executing actual
HTTP requests tou your controllers using a 'HalApiClient' instance.
