...
class LambdaLinkBuilderImplTest {
        ...
        @Test
        void should_handle_empty_list_variables_java21_workaround() {

                Link link = new LambdaLinkBuilderImpl("/items")
                                .addQueryVariable("emptyList", java.util.Collections.emptyList())
                                .build();

                assertThat(link.getHref())
                                .isEqualTo("/items{?emptyList}");
        }

        @Test
        void should_handle_empty_array_variables_java21_workaround() {

                Link link = new LambdaLinkBuilderImpl("/items")
                                .addQueryVariable("emptyArray", new String[0])
                                .build();

                assertThat(link.getHref())
                                .isEqualTo("/items{?emptyArray}");
        }
}
