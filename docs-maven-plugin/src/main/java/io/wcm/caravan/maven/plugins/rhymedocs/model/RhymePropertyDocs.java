...
public abstract class RhymePropertyDocs {

  protected final JavaProjectBuilder builder;
  protected final Class stateType;
  protected final String basePointer;

  protected RhymePropertyDocs(JavaProjectBuilder builder, Class stateType, String basePointer) {
  ...
  private static Stream<RhymePropertyDocs> createPropertyDocsRecursively(JavaProjectBuilder builder, Class<?> stateType, String basePointer,
    ...
    Stream<RhymePropertyDocs> beanProperties = DocumentationUtils.getBeanProperties(stateType)
        .map(property -> new BeanPropertyModel(builder, stateType, property, basePointer));

    Stream<RhymePropertyDocs> fieldProperties = DocumentationUtils.getPublicFields(stateType)
    ...
    Stream<RhymePropertyDocs> recursiveStream = Stream.concat(beanProperties, fieldProperties)
        .sorted(Ordering.natural().onResultOf(RhymePropertyDocs::getPropertyName))
        .flatMap(docs -> handleObjectsAndArrays(docs, processedClassNames));

    return recursiveStream;
  }

  private static Stream<RhymePropertyDocs> handleObjectsAndArrays(RhymePropertyDocs docs, Map<String, String> processedClassNames) {
    ...
    if (isSerialisedAsJsonArray(propertyType)) {
      ...
      Class<?> elementType = docs.getGenericType();
      if (elementType != null) {
  ...
  static class FixedPropertyModel extends RhymePropertyDocs {
  ...
  static class BeanPropertyModel extends RhymePropertyDocs {
    ...
    @Override
    protected Class<?> getGenericType() {
      return getTypeArgument(readMethod.getGenericReturnType());
    }
  ...
  static class FieldPropertyModel extends RhymePropertyDocs {
  ...
  static Class<?> getTypeArgument(Type type) {
    ...
    Type typeArgument = observableType.getActualTypeArguments()[0];
