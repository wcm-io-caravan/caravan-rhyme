...
class RelatedResourceHandler implements Function<HalResource, Observable<Object>> {
  ...
  RelatedResourceHandler(HalApiMethodInvocation invocation, HalApiAnnotationSupport annotationSupport,
      HalApiClientProxyFactory proxyFactory) {
  ...
  public Observable<Object> apply(HalResource contextResource) {
    ...
    // check which relation should be followed and what type of objects the
    // Observable emits
    ...
    if (!isHalApiInterface(relatedResourceType, annotationSupport)
      ...
      throw new HalApiDeveloperException(
          "The method " + invocation + " has an invalid emission type " + relatedResourceType.getName() +
  ...
  private Observable<Object> getEmbedded(String relation, Class<?> relatedResourceType,
      List<HalResource> embeddedResources, List<Link> links) {

    log.trace("{} embedded resources with relation {} were found in the context resource", embeddedResources.size(),
        relation);

    return createProxiesFromEmbeddedResources(relatedResourceType, embeddedResources, links);
  }

  private Observable<Object> getLinked(HalApiMethodInvocation invocation, String relation, Class<?> relatedResourceType,
      List<HalResource> embeddedResources,
      List<Link> links) {
    ...
    if (!variables.isEmpty()) {
      // if null values were specified for all method parameters, we assume that the
      // caller is only interested in the link templates
      if (invocation.isCalledWithOnlyNullParameters()) {
        return createProxiesFromLinkTemplates(relatedResourceType, relevantLinks);
      }

      // ignore any resolved links, and only consider link templates that contain all
      // variables specified
      ...
      if (relevantLinks.isEmpty()) {
        return Observable.error(new HalApiDeveloperException(
            "No matching link template found with relation " + relation + " which contains the variables "
                + variables.keySet()
                + " required for the invocation of " + invocation.toString()));
      }
    } else {
      // if the method being called doesn't contain parameters for template variables,
      // then link templates should be ignored
  ...
  private static List<Link> getOnlyLinksToResourcesThatArentAlreadyEmbedded(List<Link> links,
      List<HalResource> embeddedResources) {
  ...
  private Observable<Object> createProxiesFromEmbeddedResources(Class<?> relatedResourceType,
      List<HalResource> embeddedResources, List<Link> links) {
    ...
    return Observable.fromIterable(embeddedResources)
        .map(embeddedResource -> {
          ...
          // if the embedded resource is also linked, we want to make the original
          // (possibly named) link available
          // for extraction by the ResourceLinkHandler, but otherwise we just use the self
          // link
  ...
  private Observable<Object> createProxiesForLinkedHalResources(Class<?> relatedResourceType, List<Link> links,
    ...
    return Observable.fromIterable(links)
        ...
        .map(link -> {
          ...
          // if the method returns a link, then there is no need for a proxy, but we can
          // return it directly
          if (Link.class.equals(relatedResourceType)) {
            return link;
          }

          // otherwise create a new proxy implementing the HalApiInterface of the link
          // target
